...
contract CLPD is ERC20, Ownable, FunctionsClient {
    ...
    // Add a function to update the name and symbol
    function setTokenDetails(string calldata newName, string calldata newSymbol) external onlyAgent {
        _tokenName = newName;
        _tokenSymbol = newSymbol;
        emit TokenDetailsUpdated(newName, newSymbol);
    }
